...
1. `[rental_mobil_daftar]` - Menampilkan daftar kendaraan dengan filter
   * Parameter: `jumlah`, `merk`, `transmisi`, `bahan_bakar`, `tipe`, `orderby`, `order`
   * Contoh: `[rental_mobil_daftar jumlah="6" orderby="meta_value_num" order="ASC"]`

2. `[rental_mobil_detail id="ID"]` - Menampilkan detail kendaraan
   * Parameter: `id` (ID kendaraan)
   * Contoh: `[rental_mobil_detail id="123"]`
...
4. Isi harga sewa per hari
5. Klik 'Terbitkan'

### Menampilkan Daftar Kendaraan

Tambahkan shortcode `[rental_mobil_daftar]` ke halaman atau post untuk menampilkan daftar kendaraan dengan filter.

### Menampilkan Detail Kendaraan

Tambahkan shortcode `[rental_mobil_detail id="ID"]` ke halaman atau post untuk menampilkan detail kendaraan. Ganti "ID" dengan ID kendaraan yang ingin ditampilkan.
...
Filter kendaraan otomatis ditampilkan saat Anda menggunakan shortcode `[rental_mobil_daftar]`. Anda dapat menambahkan merk, transmisi, bahan bakar, dan tipe kendaraan melalui menu 'Kendaraan'.
